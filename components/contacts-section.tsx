...
export default function ContactsSection() {
    return (
        <div className="flex flex-col gap-1 h-1/4 lg:mt-7">
            <span className="text-xl font-bold">contacts</span>

            <Card className="flex items-center justify-between gap-3">
                <ContactCard
                    link="https://github.com/francescodelmonaco"
                    children={<Github className="w-5 lg:w-10 h-5 lg:h-10" />}
                />
                <ContactCard
                    link="https://linkedin.com/in/francescodelmonaco"
                    children={<Linkedin className="w-5 lg:w-10 h-5 lg:h-10" />}
                />
                <ContactCard
                    link="/documents/cv-francesco-delmonaco.pdf"
                    children={<FileUser className="w-5 lg:w-10 h-5 lg:h-10" />}
                />
                <ContactCard
                    link="mailto:francescodelmonaco1999@gmail.com"
                    children={<Mail className="w-5 lg:w-10 h-5 lg:h-10" />}
                />
            </Card>
        </div>
    )
